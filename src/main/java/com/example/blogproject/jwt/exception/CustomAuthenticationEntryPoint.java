package com.example.blogproject.jwt.exception;
...
public class CustomAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    private void handlePageResponse(HttpServletRequest request, HttpServletResponse response, String exception) throws IOException {
        ...
        if (exception != null) {
            // Additional logic to handle different exceptions
            if (exception.contains("Unauthorized")) {
                request.getSession().setAttribute("errorMessage", "You are not authorized to access this page.");
            } else if (exception.contains("BadRequest")) {
                request.getSession().setAttribute("errorMessage", "Bad request. Please check your input and try again.");
            } else {
